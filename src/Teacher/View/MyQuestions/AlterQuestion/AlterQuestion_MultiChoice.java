...
public class AlterQuestion_MultiChoice extends JPanel{
    BackgroundButton deleteBtn;
    private Question_MultiChoice question_multiChoice;
    Container warningArea=new Container();
    String[] answer ;
    public AlterQuestion_MultiChoice(int id,String stemText,String optAText,String optBText,String optCText,String optDText,int difficulty,int mark,String answerText ){
        ...
        answer= JSON.parseObject(answerText,String[].class);
        if (answer[0].equals("A")){
            optA_Box.setSelected(true);
        }
        if (answer[1].equals("B")){
            optB_Box.setSelected(true);
        }
        if (answer[2].equals("C")){
            optC_Box.setSelected(true);
        }
        if (answer[3].equals("D")){
            optD_Box.setSelected(true);
        }
        ...
        submitBtn.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (stem.getText().isEmpty()||optA.getText().isEmpty()||optB.getText().isEmpty()||optC.getText().isEmpty()||optD.getText().isEmpty()||setMark.getText().isEmpty()||!answerSet){
                    ...
                    question_multiChoice=new Question_MultiChoice(id,stem.getText(),Integer.parseInt(setMark.getText()),setDifficultyComboBox.getSelectedIndex()+1,optA.getText()
                            ,optB.getText(),optC.getText(),optD.getText(), JSON.toJSONString(answer));
                    try {
                        SubmitQuestion_C submitQuestion_c=new SubmitQuestion_C(question_multiChoice,"CHANGE_QUESTION_MULTICHOICE");
                        if (submitQuestion_c.getResultCode()==1) {
                            HomeFrame.content.removeAll();
                            HomeFrame.content.add(new MyTabbedPane_Question(1));
                ...
                warningPanel.oKButton.addMouseListener(new MouseAdapter() {
                    ...
                    public void mouseClicked(MouseEvent e) {
                        ...
                        question_multiChoice=new Question_MultiChoice(id,stem.getText(),Integer.parseInt(setMark.getText()),setDifficultyComboBox.getSelectedIndex()+1,optA.getText()
                                ,optB.getText(),optC.getText(),optD.getText(), JSON.toJSONString(answer));
                        try {
                            SubmitQuestion_C submitQuestion_c=new SubmitQuestion_C(question_multiChoice,"DELETE_QUESTION_MULTICHOICE");
                            if (submitQuestion_c.getResultCode()==1) {
                                HomeFrame.content.removeAll();
                                HomeFrame.content.add(new MyTabbedPane_Question(1));
                                HomeFrame.content.repaint();
